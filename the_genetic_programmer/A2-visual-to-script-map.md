...
Companion to `A2-sprint-review-script.md` (~4:39 at 128 wpm). Cells are named by the banner comment on their
...
* **Figures verified against commit `3814317` (30-run sweep).** Winner: 0 hidden
  nodes, 4 connections, all four sensors wired, 5 parameters. M1 re-evaluation
  80% (159/200). Sweeps at n=30: fixed GA median 18% (7/30 above 50%), NEAT
  median 51% (15/30). Species: 14/30 runs never split. `0.91667` and the 2.0
  threshold do not move.
* **No sensor was dropped this run.** All four inputs are wired, so there is no
  hollow node and no "declined a sensor" line to deliver. Part 2 is written to
  work either way.
* **Pacing buffer.** Script runs 4:15–4:58 across 140–120 wpm. At 128 — the pace the AI
  Arcade script was actually delivered at — it is 4:39, so do not ad-lib. The three prompts are
